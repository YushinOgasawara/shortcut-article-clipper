...
def generate_article_markdown(url: str) -> str:
    ...
    # プロンプトの作成
    prompt = f"""あなたはウェブコンテンツ（記事・スライド等）を正確に読み取り、構造化されたMarkdownノートを作成する専門家です。

以下のURLのコンテンツを読み取り、その内容をもとに後で見返しやすいMarkdownファイルを作成してください。

**対象URL**: {url}

# 重要な指示

1. **このURLのコンテンツを読み取り、読み取れた範囲で正確に要約してください。**
2. **タイトルは、元のコンテンツに書かれているタイトルをそのまま使用してください。**
3. **出力はMarkdownのみです。余計な説明文や前置きは一切不要です。**
4. **「はい、わかりました」などの応答文は絶対に含めないでください。**
5. **最初の文字は必ず「#」（見出し）から始めてください。**
6. **文字化けや読み取りにくい部分があっても、読み取れた情報を最大限活用して要約してください。**
