...
def generate_article_markdown(url: str) -> str:
    ...
    # プロンプトの作成
    prompt = f"""あなたはウェブ記事を正確に読み取り、構造化されたMarkdownノートを作成する専門家です。

以下のURLの記事を**必ず正確に読み取り**、その内容をもとに後で見返しやすいMarkdownファイルを作成してください。

**対象URL**: {url}

# 絶対に守るべきルール

1. **このURLの記事内容を必ず正確に読み取ってください。想像や推測で内容を作らないでください。**
2. **記事のタイトルは、元の記事に書かれているタイトルをそのまま使用してください。**
3. **出力はMarkdownのみです。余計な説明文や前置きは一切不要です。**
4. **「はい、わかりました」「以下は記事の内容です」などの応答文は絶対に含めないでください。**
5. **最初の文字は必ず「#」（見出し）から始めてください。**
6. **記事が読めない場合は、「記事の取得に失敗しました」と出力してください。**

# 出力形式

以下の構造でMarkdownを作成してください：

# [記事のタイトル - 元の記事に書かれているものをそのまま使用]
...
## 概要
[記事の要点を2-4行で簡潔に。記事の内容に基づいて正確に記述]

## 主要なポイント
- [記事に書かれている重要な主張や発見を箇条書きで3-7個]
- [記事内の具体的な数字やデータがあれば含める]

## 詳細メモ
[記事内の特に興味深い点、技術的な詳細、引用など]

## タグ
[記事の内容に応じた適切なタグを3-5個、例: #AI #プログラミング #ビジネス]
